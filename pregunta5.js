...
function nuevaLabel(){
  beers.forEach(beer => {
    //let nombre = beer.name;
    beer.label.replace('s3.amazonaws.com/brewerydbapi','tecnoshare.sharepoint.com/sites');
    let nvo = beer.label.split("/").pop()
    nvo.push(beer.name + '.png');
  });
  return beers
}
